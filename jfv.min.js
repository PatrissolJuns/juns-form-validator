const MARQUEE_BEGIN="${";const MARQUEE_END="}";const validationCLass={input:['jfv-input-valid','jfv-input-invalid','jfv-input-information'],label:['jfv-label-valid','jfv-label-invalid','jfv-label-information']};(function(old){$.fn.attr=function(){if(arguments.length===0){if(this.length===0)return null;let obj={};$.each(this[0].attributes,function(){if(this.specified){obj[this.name]=this.value}});return obj}
    ...
    return[]}})($.fn.jfv);const JFV={Extractor:class{constructor(str){this.str=str}
        ...
        setLanguage(language){const LanguageError="Unknown language "+language;try{if(Object.keys(this.constructor.errorJSON).includes(language)){this.currentLanguage=language}
            ...
            return this}}};const JFV_ERROR_MESSAGE=new JFV.ErrorMessage();function emptyChecker(element){if(element.val().length>510)return[!1,`Please enter at most 510 character(s)`];else return[!0,""]}
function emailChecker(element){let regex=/^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,6}$/i;return regex.test(element.val())===!0?[!0,""]:[!1,JFV_ERROR_MESSAGE.getValueOfKey('email')]}
...
function numberChecker(element){let numberResult=/^[+-]?\d+(\.\d+)?$/.test(element.val())?[!0,""]:[!1,JFV_ERROR_MESSAGE.getValueOfKey('number.number')];let rangeResult=[!0,""],minLength=Number(element.jfv("min")),maxLength=Number(element.jfv("max"));let length=Number(element.val());let errorMessage=new JFV.Extractor('');if(minLength||maxLength){if(minLength&&maxLength)
    rangeResult=(length>=minLength&&length<=maxLength)?[!0,""]:[!1,errorMessage.reset(JFV_ERROR_MESSAGE.getValueOfKey('number.range')).setValue('minLength',minLength).setValue('maxLength',maxLength).getValue];else if(minLength)
    rangeResult=(length>=minLength)?[!0,""]:[!1,errorMessage.reset(JFV_ERROR_MESSAGE.getValueOfKey('number.min')).setValue('minLength',minLength).getValue];else rangeResult=(length<=maxLength)?[!0,""]:[!1,errorMessage.reset(JFV_ERROR_MESSAGE.getValueOfKey('number.max')).setValue('maxLength',maxLength).getValue]}
else{let equalLength=element.jfv("equal");if(equalLength){rangeResult=(length<=maxLength)?[!0,""]:[!1,errorMessage.reset(JFV_ERROR_MESSAGE.getValueOfKey('number.equal')).setValue('equalLength',equalLength).getValue]}}
...
function requiredChecker(element){return element.val().length<=0?[!1,JFV_ERROR_MESSAGE.getValueOfKey('required')]:[!0,""]}
function minMaxCheckerCharacter(element){let length=Number(element.val().length);let minLength=Number(element.jfv("min"));let maxLength=Number(element.jfv("max"));let errorMessage=new JFV.Extractor('');if(length<minLength)return[!1,errorMessage.reset(JFV_ERROR_MESSAGE.getValueOfKey('character.min')).setValue('minLength',minLength).getValue];else if(length>maxLength)return[!1,errorMessage.reset(JFV_ERROR_MESSAGE.getValueOfKey('character.max')).setValue('maxLength',maxLength).getValue];else return[!0,""]}
function equalChecker(element){let length=Number(element.val().length);let equalLength=Number(element.jfv("equal"))?Number(element.jfv("equal")):2;let errorMessage=new JFV.Extractor(JFV_ERROR_MESSAGE.getValueOfKey('character.max'));return length===equalLength?[!0,""]:[!1,errorMessage.setValue('equalLength',equalLength).getValue]}
function passwordConfirmationChecker(element){let password=$('#'+(element.jfv("ref")?element.jfv("ref"):'password'));if(element.val()!==password.val())return[!1,JFV_ERROR_MESSAGE.getValueOfKey('password.confirmation')];else return[!0,""]}
